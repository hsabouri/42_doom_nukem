...
t_sdl *sdl)
{
	...
	sector = &game.sectors[player.my_entity.physic.sector_id];
	sel = world_selector(game);
	print_selected(game, sel, sdl);
	if (game.player.my_entity.physic.look_v >= 0 && events.keys[SDL_SCANCODE_KP_PLUS])
		*sector = sector_height(game, *sector, PART_CEILING, 0.1);
	if (game.player.my_entity.physic.look_v >= 0 && events.keys[SDL_SCANCODE_KP_MINUS])
		*sector = sector_height(game, *sector, PART_CEILING, -0.1);
	if (game.player.my_entity.physic.look_v < 0 && events.keys[SDL_SCANCODE_KP_PLUS])
