...
/*   Updated: 2019/06/19 12:46:26 by hsabouri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_game			assign_entity(t_assign_entity *state, t_game game)
{
	...
	if (*state->entity >= 0 && *state->entity < (ssize_t)game.nentities)
		game.entities[*state->entity].data = sector;
	else if (*state->spawn >= 0 && *state->spawn < (ssize_t)game.nentities)
		game.entities[*state->spawn].data = sector;
	*state->entity = -1;
	*state->spawn = -1;
	state->walls[1] = -1;
