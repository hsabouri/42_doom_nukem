...
t_bunch			filter(t_game game, t_sector sector, t_vec2 r[2], t_vec2 pos,\
ssize_t mask)
{
	t_bunch		res;
	t_i_wall	current;
	...
	while (current.wall_id < sector.start + sector.number)
	{
		...
		if ((size_t)mask != current.wall_id &&
		!((vec2_cross(r[0], a).z >= 0 && vec2_cross(r[0], b).z >= 0) ||
		(vec2_cross(r[1], a).z <= 0 && vec2_cross(r[1], b).z <= 0)))
		{
