...
static int			quit_game(void *parent_state)
{
	t_menu_state *menu;
...
t_menu_state *state)
{
	...
	current.last_render = SDL_CreateTexture(sdl->renderer, SDL_PIXELFORMAT_RGBA32,
		SDL_TEXTUREACCESS_STREAMING, current.size.x, current.size.y);
